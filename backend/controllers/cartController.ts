...
export const getCart = async (
  ...
  try {
    ...
    const cart = await Cart.findOne<CartDocument>({ user: userId }).populate(
      "items.product"
    );
...
export const addToCart = async (
  req: Request<unknown, unknown, AddCartBody>,
  res: Response
): Promise<Response> => {
  try {
    ...
    const cart = await Cart.findOne<CartDocument>({ user: userId });
    if (!cart) {
...
export const updateCartItem = async (
  req: Request<CartParams & { productId: string }, unknown, UpdateCartBody>,
  res: Response
): Promise<Response> => {
  try {
    ...
    const cart = await Cart.findOne<CartDocument>({ user: new Types.ObjectId(userId) }).populate(
      "items.product"
    );
...
export const removeCartItem = async (
  req: Request<CartParams & { productId: string }>,
  res: Response
): Promise<Response> => {
  try {
    const { userId, productId } = req.params;
    const cart = await Cart.findOne<CartDocument>({ user: new Types.ObjectId(userId) });
    if (!cart) {
      return res.status(404).json({ message: "Cart not found for this user." });
    }
...
export const clearCart = async (
  ...
  try {
    const cart = await Cart.findOne<CartDocument>({ user: new Types.ObjectId(req.params.userId) });
    if (!cart) {
      return res.status(404).json({ message: "Cart not found for this user." });
    }
